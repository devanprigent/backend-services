-- Your tables, indexes, and constraints (INTERVIEW_EXERCISE.md)

CREATE TABLE clinic (
    clinic_id SERIAL PRIMARY KEY,
    clinic_name VARCHAR(255),
    opening_time INT,
    closing_time INT,
    slot_duration INT
);

CREATE TABLE patient (
    patient_id SERIAL PRIMARY KEY
);

-- We assume an appointment is only one slot
CREATE TABLE appointments (
    appointment_id SERIAL PRIMARY KEY,
    patient_id INT NOT NULL,
    clinic_id INT NOT NULL,
    start_time TIMESTAMPTZ NOT NULL,
    appointment_type VARCHAR(255),
    appointment_status VARCHAR(255) NOT NULL DEFAULT 'CREATED',
    FOREIGN KEY (patient_id) REFERENCES patient(patient_id),
    FOREIGN KEY (clinic_id) REFERENCES clinic(clinic_id)
);

-- Only active appointments occupy a slot (canceled ones free it for rebooking)
CREATE UNIQUE INDEX appointments_active_slot_unique
    ON appointments (clinic_id, start_time)
    WHERE appointment_status = 'CREATED';
